---
lab:
  title: "ラボ: Enabling Continuous Integration with Azure Pipelines"
...
1.  「**Create a new project**」 ページの 「**New Project Name**」 テキストボックスに「**Enabling Continuous Integration with Azure Pipelines**」と入力し、「**Select organization**」 ドロップダウン リストで、Azure DevOps 組織を選択して、「**Choose a template**」 をクリックします。
...
1.  前の手順で生成した **Enabling Continuous Integration with Azure Pipelines**プロジェクトで Azure DevOps 組織を表示している Web ブラウザーで、垂直ナビゲーション ペインで、「**Pipelines**」 セクションを選択し、「**Pipelines**」 ビューが表示されていることを確認します。
...
1.  「**Pipelines**」 ペインで、既存の **PartsUnlimitedE2E** パイプラインを表すエントリにマウス ポインターを合わせると、右側に省略記号が表示されます。
1.  省略記号をクリックし、ドロップダウン メニューで 「**Edit**」 をクリックします。

    > **注**: ラボの後半でトリガーされる 2 つのパイプラインを回避するには、テンプレートで作成されたパイプラインの CI トリガーを無効にすることから始めます。

1.  **PartsUnlimitedE2E** ペインの 「**Tasks**」 タブで、「**Triggers**」 タブをクリックし、「**Enable continuous integration**」 チェックボックスをオフにして、「**Save & queue**」 をクリックし、「**Save**」 をクリックします。
1.  新しいパイプラインを作成するには、Azure DevOps ポータルの垂直ナビゲーション ペインで 「**Pipelines**」 を選択して、「**Pipelines**」 ビューに戻ります。
1.  「**Pipelines**」 ペインに戻り、「**New pipeline**」 をクリックして、新しいビルド パイプラインを作成します。
...
1.  「**Choose a template**」 ペインの 「**Search**」 テキスト ボックスに「**ASP.NET**」と入力し、結果のリストで **ASP.NET** テプレートを選択し、「**Apply**」 をクリックしてこのテンプレートをビルド定義に適用します。
...
1.  「**Variables**」 タブを選択し、その内容を確認します。

    > **注**: ここでは、構成やプラットフォームなど、ビルド中に使用される特別なパラメーターを構成できます。

1.  「**Triggers**」 タブを選択し、「**Enable continuous integration**」 チェックボックスをオンにします。
...
1.  「**Save & queue**」 タブ ヘッダーを選択し、ドロップダウン メニューで 「**Save & queue**」 エントリを選択して、新しいビルドを保存してキューに入れます。

    > **注**: 注：パイプラインの成果物の保持期間は、「Project settings」→「Settings」→「Retention policy」から定義することができます。これらの設定により、どのパイプラインランのをどのくらいの期間保持するかを設定することができます。
    > 
1.  「**Run pipeline**」 ペインで、デフォルト オプションを受け入れ、「**Save and run**」 をクリックします。これにより、パイプライン実行ジョブの 「**Summary**」 タブが 「**Queued**」 ステータスで自動的に表示されます。
...
1.  パイプライン実行ジョブの 「**Summary**」 タブの 「**Jobs**」 セクションで、 「**Agent job 1**」 をクリックします。これにより、ジョブの詳細ペインが表示されます。
...
1.  このビルドのテストを確認するには、「**Tests**」 タブを選択します。
...
1.  垂直ナビゲーション ペインで、「**Pipelines**」 セクションを選択し、「**Pipelines**」 ビューが表示されていることを確認します。
1.  「**Pipelines**」 ペインで、変更によってトリガーされた新しいビルドを表すエントリが含まれていることを確認します (その番号には末尾の **.2** が含まれていることに注意してください)。
